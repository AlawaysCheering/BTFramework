...
namespace Framework.Common.StateMachine
{
       ...
       where TState : MonoBehaviour, IState
    {
        ...
        public void LaunchStateMachine()
        {
            if (!_build)
            {
                DebugUtil.LogError($"状态机{Name}未构建，无法启动");
            }

            if (!Root)
            {
                DebugUtil.LogError($"状态机{Name}不是根状态机，无法启动");
            }
        ...
        public bool SwitchState(TState state, bool allowSwitchToDefaultWhenError)
        {
            ...
            if (!IsChildState(state))
            {
                ...
                {
                    DebugUtil.LogError($"状态机{Name}不存在状态({state.Name})，切换至默认状态{defaultState}");
                    return SwitchToDefault();
                }
                DebugUtil.LogError($"状态机{Name}不存在状态({state.Name})，无法切换状态");
                return false;
            }
            // 如果状态机失活就不能切换到下一状态，防止错误调用API
            if (!Active)
            {
                DebugUtil.LogError($"状态机{Name}失活，切换状态失败");
                return false;
            }
            // 如果被切换的状态不允许进入就不能切换到下一状态
            if (!state.AllowEnter(currentState))
            {
                DebugUtil.LogError($"状态{state.Name}不允许进入，切换状态失败");
                return false;
            }
        ...
        bool IStateMachine.SwitchState(Framework.Common.StateMachine.IState state, bool allowSwitchToDefaultWhenError) => SwitchState(state as TState, allowSwitchToDefaultWhenError);   
        public bool SwitchState(string stateName,bool allowSwitchToDefaultWhenError)
        {
            if (_states.TryGetValue(stateName, out var newState))
                return SwitchState(newState, allowSwitchToDefaultWhenError);
            return false;
        }
        ...
        private bool IsChildState(IState state)
        {
        ...
        protected virtual void OnExit(TState nextState)
        {
            ...
            if (currentState)
            {
                ...
                // 如果存在激活状态且下一状态为其他状态机状态，则说明当前状态机不是激活状态机，
                // 应将当前状态清空，防止下次重新进入状态机恢复之前的旧状态
                if (!state)
                {
                    currentState=null;
                }
        ...
        void IState.Exit(Framework.Common.StateMachine.IState nextState)=>OnExit(nextState as TState);
    }
}